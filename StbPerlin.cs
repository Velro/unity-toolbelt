...
//
// Unity conversion by James Fulop
//
// LICENSE
//
...
public class StbPerlin
{
	...
	static int[] stb__perlin_randtab =
	{
	...
	static float stb__perlin_grad(int hash, float x, float y, float z)
	{
	   ...
	   int[] indices =
	   {
		...
		float gradX = basis[indices[hash & 63], 0];
		float gradY = basis[indices[hash & 63], 1];
		float gradZ = basis[indices[hash & 63], 2];
		return gradX* x + gradY* y + gradZ* z;
	}

	static float stb__perlin_ease(float a)
	{
		return ((a * 6 - 15) * a + 10) * a * a * a;
	}

	public static float Noise3(float x, float y, float z, uint x_wrap, uint y_wrap, uint z_wrap)
	{
		float u, v, w;
		...
		uint x_mask = (x_wrap - 1) & 255;
		uint y_mask = (y_wrap - 1) & 255;
		uint z_mask = (z_wrap - 1) & 255;
